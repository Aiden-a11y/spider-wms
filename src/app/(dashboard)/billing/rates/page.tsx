    ...
    <div className="p-8 max-w-5xl">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div className="flex-1">
          ...
          <p className="text-slate-400 text-xs mt-0.5">고객사별 커스텀 요금을 설정합니다. 저장된 요금은 인보이스 생성 시 자동 적용됩니다.</p>
        </div>
      </div>
            ...
            <div className="text-xs text-slate-400 pb-2.5">
              {loadingMaster ? (
                ...
                <span className="text-emerald-600">
                  ✓ 저장된 rate master 있음 ({new Date(masterUpdatedAt).toLocaleDateString("ko-KR")})
                </span>
              ) : (
                <span className="text-slate-400">기본 rate 표시 중 (저장된 마스터 없음)</span>
              )}
            </div>
          )}
        </div>
      ...
      {selectedCustomer && !loadingMaster && (
        <>
                            ...
                            return (
                              ...
                              >
                                ...
                                <td className="px-4 py-2.5 text-right">
                                  {item.costPlus ? (
                                    <span className="text-xs text-slate-400 italic">고정</span>
                                  ) : (
          ...
          <p className="text-xs text-slate-400 mt-3 text-right">
            기본값(Default Rate)을 그대로 사용하려면 Custom Rate를 비워두세요. 저장된 마스터 기준: <span className="font-mono">{RATE_VERSION}</span>
          </p>
        </>
      )}
      ...
      {!selectedCustomer && (
        <div className="text-center py-20 text-slate-400">
          ...
          <p className="text-sm">고객사를 선택하면 요금 설정이 표시됩니다.</p>
        </div>
      )}
    </div>
  );
